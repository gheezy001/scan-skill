...
export default function CollaborateursPage() {
  ...
  const [form, setForm] = useState({
    ...
    contactUrgenceNom:'', contactUrgenceTel:'', photo:'',
  });
  const [habForm, setHabForm] = useState({ typeId:'', dateObtention:'', dateExpiration:'', entreprise:'', document:'' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setForm({ nom:'',prenom:'',telephone:'',email:'',role:'',entreprise:'',dateEmbauche:'',adresse:'',nationalite:'',groupeSanguin:'',numeroPieceIdentite:'',typePieceIdentite:'CIN',contactUrgenceNom:'',contactUrgenceTel:'',photo:'' });
    load();
  };
  ...
  const handleAddHab = async (e: React.FormEvent) => {
    ...
    setHabForm({ typeId:'',dateObtention:'',dateExpiration:'',entreprise:'',document:'' });
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-5">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="col-span-2">
              ...
              <FileUpload
                folder="collaborateurs"
                accept=".jpg,.jpeg,.png,.webp"
                label="Ajouter une photo (JPG, PNG)"
            ...
            : collaborateurs.map(c => {
              ...
              return (
                <div key={c.id} onClick={()=>setSelected(c)}
                  ...
                  {c.photo
                    ? <img src={c.photo} alt={c.nom} className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 overflow-y-auto max-h-[80vh]">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                {selected.photo
                  ? <img src={selected.photo} alt={selected.nom} className="w-14 h-14 rounded-xl object-cover" />
              ...
              <div className="flex gap-2 flex-wrap justify-end">
                ...
                <button onClick={()=>handleDelete(selected.id)}
                  className="text-xs border border-red-200 text-red-600 px-3 py-1.5 rounded-lg">Supprimer</button>
              </div>
            </div>
            ...
            <div>
                  ...
                  <div key={h.id} className={`p-3 rounded-xl border ${h.statut==='VALIDE'?'bg-green-50 border-green-200':'bg-red-50 border-red-200'}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        {h.document && (
                          <a href={h.document} target="_blank" rel="noopener noreferrer"
                            className="text-xs text-blue-600 hover:underline">📎 Voir le document</a>
                        )}
                      </div>
              ...
              <form onSubmit={handleAddHab} className="mt-3 p-3 bg-slate-50 rounded-xl space-y-2">
                ...
                <FileUpload
                  folder="habilitations"
                  label="Document habilitation (PDF, image)"
                  currentUrl={habForm.document}
                  onUpload={(url)=>setHabForm({...habForm,document:url})}
                />

...
function Badge({ statut }: { statut: string }) {
  const m: Record<string,string> = {
    ...
    EN_MAINTENANCE:'bg-yellow-100 text-yellow-800', HORS_SERVICE:'bg-red-100 text-red-800'
  };
...
export default function AppareillagesPage() {
  ...
  const [form, setForm] = useState({ nom:'', reference:'', type:'', localisation:'', statut:'DISPONIBLE', documentationTechnique:'' });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setForm({ nom:'',reference:'',type:'',localisation:'',statut:'DISPONIBLE',documentationTechnique:'' });
    load();
  };
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-5">
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              {[['Nom *','nom'],['Référence *','reference'],['Type *','type'],['Localisation','localisation']].map(([label,key])=>(
                <div key={key}>
                  <label className="text-xs text-slate-500 mb-1 block">{label}</label>
                  <input required={label.includes('*')} value={(form as any)[key]} onChange={e=>setForm({...form,[key]:e.target.value})}
            ...
            <div>
              ...
              <FileUpload
                folder="appareillage"
                label="Ajouter la documentation technique (PDF, Word)"
                currentUrl={form.documentationTechnique}
                onUpload={(url)=>setForm({...form,documentationTechnique:url})}
              />
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          {appareils.map(a=>(
            <div key={a.id} onClick={()=>setSelected(a)}
              className={`flex items-center justify-between px-4 py-3 border-b border-slate-100 cursor-pointer hover:bg-slate-50 ${selected?.id===a.id?'bg-blue-50':''}`}>
              <div>
                <p className="font-medium text-sm">{a.nom}</p>
                <p className="text-xs text-slate-400 font-mono">{a.reference} · {a.type}</p>
              </div>
        ...
        {selected && (
          <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-4 overflow-y-auto max-h-[80vh]">
            <div className="flex items-start justify-between">
              ...
              <div className="flex gap-2">
                <a href={`data:text/plain,${encodeURIComponent(qrValue(selected.id))}`} download={`qr-${selected.reference}.txt`}
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[['Référence', selected.reference],['Localisation', selected.localisation||'—'],['Dernière révision', fmt(selected.dateDerniereRevision)]].map(([l,v])=>(
                <div key={l} className="bg-slate-50 px-3 py-2 rounded-lg">
                  <p className="text-slate-400">{l}</p><p className="text-slate-700 font-medium mt-0.5">{v}</p>
                </div>
              ))}
            </div>
            {selected.documentationTechnique && (
              <a href={selected.documentationTechnique} target="_blank" rel="noopener noreferrer"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['DISPONIBLE','EN_SERVICE','EN_MAINTENANCE','HORS_SERVICE'].map(s=>(
                  <button key={s} onClick={()=>handleUpdateStatut(selected.id,s)}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${selected.statut===s?'bg-slate-800 text-white border-slate-800':'border-slate-200 text-slate-600 hover:border-slate-400'}`}>
                    {s.replace(/_/g,' ')}
                  </button>
                ))}
              </div>
